 ...
 */
export function transformDynamicImports(options = {}) {
    const { resourceBaseVar = defaultResourceBaseVar, entryNamePredicate = defaultEntryNamePredicate, enableSourceMap = false, widgetPlaceholder = '{{widget.wid}}', chunkFilePattern = '.chunk.js', entryFileName = 'main.js', staticImportUrlTemplate = '{{site.url}}/widget_manager/{{widget.wid}}/{{widget.version}}.js', } = options;
    return {
        ...
        generateBundle(_outputOptions, bundle) {
            ...
            for (const [fileName, chunkOrAsset] of Object.entries(bundle)) {
                ...
                if (entryNamePredicate(chunk)) {
                    ...
                    const dynamicImportRegex = new RegExp(`import\\(\\s*([\\x60'\"])(\\.\/[^\\x60'\"()]+?${escapedChunkPattern})\\1\\s*\\)`, 'g');
                    let match;
                ...
                if (fileName.includes(chunkFilePattern)) {
                    ...
                    const staticImportRegex = new RegExp(`from\\s*([\\x60'\"])\\.\/${escapedEntryFile}\\1`, 'g');
                    let match;
                ...
                if (transformCount > 0) {
                    ...
                    this.warn(`Transformed ${transformCount} import(s) in ${fileName}`);
                }
            }
            if (totalTransformations === 0) {
                this.warn('No dynamic imports were transformed. This may be expected if there are no code-split chunks, ' +
                    'or it could indicate that the regex patterns do not match the generated code.');
            }
            else {
                this.warn(`Total transformations applied: ${totalTransformations}`);
            }
        },
    };
}
